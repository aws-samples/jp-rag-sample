...
const InputWithSuggest: React.FC = () => {
  ...
  // local 変数
  const [suggestedQueryOptions, setFilteredOptions] = useState(
    TOP_QUERIES.slice(0, MAX_QUERY_SUGGESTION)
  ); // AIによるサジェストクエリ
    ...
    try {
      ...
      if (data) {
        setFilterOptions([
          filterOptions[0], // 言語設定
          filterOptions[1], // ソート順序
          ...getFiltersFromQuery(data, datasourceInfo),
        ]); // クエリから受け取ったフィルタ候補
      }
      return data;
    } catch (err) {
  ...
  const research = async (queryText: string) => {
    ...
    // 引用候補を生成
    const streamQuote = await infStreamClaude(
      createQuotePrompt(parsedResult, queryText)
    );
    let tmpResultQuote = '';
    ...
    const relevantSelectedInfo: AiSelectedInfo[] = Array.from(
      relevantSelectedInfoMap.values()
    );
    ...
    if (relevantSelectedInfo.length <= 0) {
      ...
      const streamFinalAnswer = await infStreamClaude(
        createFinalAnswerPrompt(relevantSelectedInfo, queryText)
      );
      // let tmpResultAnswer = '';
    ...
    const streamNextQuery = await infStreamClaude(
      createNextQeuryPrompt(tmpResultQuote, queryText)
    );
    let tmpNextQuery = '';
  ...
  const handleKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'Tab') {
      ...
      setCurrentFocus((prevFocus) => {
        const totalOptions =
          recentQueryList.length + suggestedQueryOptions.length;
        if (prevFocus === -1) {
          return 0; // 最初の候補を選択
    ...
    if (currentFocus >= 0) {
      const selectedOption =
        currentFocus < recentQueryList.length
        ...
        <div
          ...
          {suggestedQueryOptions.map((option, index) => (
            <HStack
              ...
              onClick={() => handleOptionClick(option)}
              onMouseEnter={() =>
                setCurrentFocus(index + recentQueryList.length)
              } // マウスカーソルが乗ったものを記録
              onMouseLeave={() => setCurrentFocus(-1)} // マウスカーソルが外れたことを記録
              style={{
                backgroundColor:
                  currentFocus === index + recentQueryList.length
                    ? 'var(--chakra-colors-green-100)'
                    : 'white', // マウスカーソルが乗ったものの色を変える
                cursor: 'pointer',
                padding: '5px',
