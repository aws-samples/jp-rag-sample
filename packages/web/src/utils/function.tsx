...
export function getCurrentSortOrder(
  ...
  for (const filterOption of filterOptions) {
    if (
      filterOption.filterType === 'SORT_BY' &&
      isArrayString(filterOption.selected)
    ) {
      // 関連度 (Default) の場合は何もしない
      if (
        filterOption.selected[SORT_ATTRIBUTE_INDEX] === DEFAULT_SORT_ATTRIBUTE
      ) {
        return undefined;
      }
      return {
        ...
        SortOrder:
          filterOption.selected[SORT_ORDER_INDEX] === 'ASC'
            ? SortOrder.ASC
            : SortOrder.DESC,
      };
    }
...
export function getAttributeFilter(filterOptions: Filter[]): AttributeFilter {
  ...
  for (const filterOption of filterOptions) {
    // 言語設定
    if (
      filterOption.filterType === 'LAUNGUAGE_SETTING' &&
      isArrayString(filterOption.selected)
    ) {
      fs.push({
        EqualsTo: {
      ...
      filterOption.filterType === 'SELECT_MULTI_STRING_FROM_LIST' &&
      isArrayBoolean(filterOption.selected)
    ) {
      /*
       * 文字列複数選択式
       */
      ...
      filterOption.filterType === 'CONTAIN_STRING' &&
      isArrayString(filterOption.selected)
    ) {
      /*
       * 文字列自由記述式
      ...
      filterOption.filterType === 'RANGE_NUM' &&
      isArrayNumber(filterOption.selected)
    ) {
      /*
       * 数値範囲
       */
      ...
      // 変更されたときのみ
      if (
        filterOption.selected[MIN_INDEX].toString() !==
          filterOption.options[MIN_INDEX].value ||
        filterOption.selected[MAX_INDEX].toString() !==
          filterOption.options[MAX_INDEX].value
      ) {
        fs.push({
          AndAllFilters: [
            {
      ...
      filterOption.filterType === 'RANGE_DATE' &&
      isArrayDate(filterOption.selected)
    ) {
      /*
       * 特定の時間空間にある
       */
      ...
      if (fr.DocumentAttributeValueType === 'STRING_VALUE') {
        ...
        for (const valuePair of fr.DocumentAttributeValueCountPairs ?? []) {
          if (valuePair.DocumentAttributeValue?.StringValue !== undefined) {
            ...
            if (
              valuePair.DocumentAttributeValue.StringValue in datasourceInfo
            ) {
              os.push({
                name: datasourceInfo[
                  valuePair.DocumentAttributeValue?.StringValue
                ],
                value: '',
              });
            } else {
        ...
        const oldestTime: Date = new Date(
          Math.min(...osDate.map((date) => date.getTime()))
        ); // 最も古い時間
        const newestTime: Date = new Date(
          Math.max(...osDate.map((date) => date.getTime()))
        ); // 最も新しい時間
...
export function getFAQWithHighlight(
  AdditionalAttributes: AdditionalResultAttribute[],
  targetName: string
): TextWithHighlights | undefined {
...
export function getNounAnswerFromExcerpt(
  textWithHighlights: TextWithHighlights | undefined
): string {
...
export function kendraResultToAiSelectedInfo(
  searchResult: QueryResult
): AiSelectedInfo[] {
  let id_num: number = 0;

  const selectedItem: AiSelectedInfo[] = [];
...
export function createQuotePrompt(
  searchResult: AiSelectedInfo[],
  query: string
): string {
  /**
   * 引用生成のためのプロンプト
